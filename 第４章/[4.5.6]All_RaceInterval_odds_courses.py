...
#フォルダ指定
dir_path = "results/RaceInterval/1/"
...
for JyoCD in JyoCDs:
	...
	for Course in Courses:
		...
		for RaceInterval in RaceIntervals:
			race_num[JyoCD][TrackCD+Kyori][RaceInterval] = 0
			tyakuKaisu[JyoCD][TrackCD+Kyori][RaceInterval] = {}
			tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval] = {}
			fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval] = {}
			tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval] = {}
			fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval] = {}

			for OddsInt in OddsInts:
				#レース結果格納する配列：総数, １着, ２着, ３着
				tyakuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] = [0] * 4
				#単勝・複勝の的中回数
				tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] = 0
				fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] = 0
				#単勝・複勝の払戻総額
				tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] = 0
				fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] = 0
		...
		for Year in Years:
			...
			for RACE in RACEs:
				...
				for UMA_RACE in UMA_RACEs:
					...
					#出走頭数
					race_num[JyoCD][TrackCD+Kyori][RaceInterval] += 1

					#レース結果を集計
					tyakuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt][0] += 1
					if( int(UMA_RACE["KakuteiJyuni"]) <= 3):
						tyakuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt][int(UMA_RACE["KakuteiJyuni"])] += 1
					#単勝的中の場合
					for i in range(3):
						n = str(i + 1)
						if(HARAI["PayTansyoUmaban" + n] == UMA_RACE["Umaban"]):
							tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] += int(HARAI["PayTansyoPay" + n])
							tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] += 1
					#複勝的中の場合
					for i in range(5):
						n = str(i + 1)
						if(HARAI["PayFukusyoUmaban" + n] == UMA_RACE["Umaban"]):
							fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] += int(HARAI["PayFukusyoPay" + n])
							fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] += 1
					#ターミナルへ出力
					#print(text)
			...
			for RaceInterval in RaceIntervals:
				for OddsInt in OddsInts:
					_tyakuKaisu = tyakuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt]
					#着外の回数
					...
					tansho_kaishuritsu = round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / b_tyakuKaisu, 1)
					#複勝の的中率と回収率
					fukusho_ritsu = round(fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / b_tyakuKaisu * 100, 1)
					fukusho_kaishuritsu = round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / b_tyakuKaisu, 1)
	...
	for Course in Courses:
		...
		for RaceInterval in RaceIntervals:	
			#列カラム名出力
			fout_tansho_kaishuritu.write( "\t" + "中" + RaceInterval + "週")
			fout_fukusho_kaishuritu.write( "\t" + "中" + RaceInterval + "週")
		...
		for OddsInt in OddsInts:
			...
			for RaceInterval in RaceIntervals:
				...
				#単勝回収率
				tansho_kaishuritsu = round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / b_shusokaisu, 1)
				fout_tansho_kaishuritu.write( "\t" + str(tansho_kaishuritsu) )
				#複勝の回収率
				fukusho_kaishuritsu = round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / b_shusokaisu, 1)
		...
		for RaceInterval in RaceIntervals:
			...
			for OddsInt in OddsInts:
				...
				total_tansho_haraimodoshi_odds += tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt]
				total_fukusho_haraimodoshi_odds += fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt]
		...
		for RaceInterval in RaceIntervals:
			fout_tansho_kaishuritu.write( "\t" + str(race_num[JyoCD][TrackCD+Kyori][RaceInterval]) )
			fout_fukusho_kaishuritu.write( "\t" + str(race_num[JyoCD][TrackCD+Kyori][RaceInterval]) )
...
for JyoCD in JyoCDs:
	...
	for Course in Courses:
		...
		for RaceInterval in RaceIntervals:
			for OddsInt in OddsInts:
				shusotosu = tyakuKaisu[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt][0]
				if(shusotosu < MinimumNumber):  continue
				N += 1
				tansho_average += tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / shusotosu
				fukusho_average += fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / shusotosu

				ranking[str(n)] = ( tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / shusotosu + fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / shusotosu ) / 2
				n += 1
...
for t in total_ranking:
	...
	text += "\t" + shibaDirt
	text += "\t" + Kyori + "m"
	text += "\t" + "中" + RaceInterval + "週"
	text += "\t" + OddsInt + "~" + str( int(OddsInt) + 1 )
	text += "\t" + str(round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / shusotosu, 1))
	text += "\t" + str(round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][RaceInterval][OddsInt] / shusotosu, 1))
