...
for JyoCD in JyoCDs:
	...
	for Course in Courses:
		...
		#種別ごと
		for SexCD in SexCDs:
			race_num[JyoCD][TrackCD+Kyori][SexCD] = 0
			tyakuKaisu[JyoCD][TrackCD+Kyori][SexCD] = {}
			tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD] = {}
			fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD] = {}
			tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD] = {}
			fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD] = {}

			for OddsInt in OddsInts:
				#レース結果格納する配列：総数, １着, ２着, ３着
				tyakuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt] = [0] * 4
				#単勝・複勝の的中回数
				tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt] = 0
				fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt] = 0
				#単勝・複勝の払戻総額
				tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] = 0
				fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] = 0
		...
		for Year in Years:
			...
			for RACE in RACEs:
				...
				for UMA_RACE in UMA_RACEs:
					...
					SexCD = UMA_RACE["SexCD"]
					#出走頭数
					race_num[JyoCD][TrackCD+Kyori][SexCD] += 1

					#レース結果を集計
					tyakuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt][0] += 1
					if( int(UMA_RACE["KakuteiJyuni"]) <= 3):
						tyakuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt][int(UMA_RACE["KakuteiJyuni"])] += 1
					#単勝的中の場合
					for i in range(3):
						n = str(i + 1)
						if(HARAI["PayTansyoUmaban" + n] == UMA_RACE["Umaban"]):
							tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] += int(HARAI["PayTansyoPay" + n])
							tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt] += 1
					#複勝的中の場合
					for i in range(5):
						n = str(i + 1)
						if(HARAI["PayFukusyoUmaban" + n] == UMA_RACE["Umaban"]):
							fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] += int(HARAI["PayFukusyoPay" + n])
							fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt] += 1
					#ターミナルへ出力
					#print(text)
			...
			for SexCD in SexCDs:
				for OddsInt in OddsInts:
					_tyakuKaisu = tyakuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt]
					#着外の回数
					...
					tansho_kaishuritsu = round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / b_tyakuKaisu, 1)
					#複勝の的中率と回収率
					fukusho_ritsu = round(fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / b_tyakuKaisu * 100, 1)
					fukusho_kaishuritsu = round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / b_tyakuKaisu, 1)
	...
	for Course in Courses:
		...
		for SexCD in SexCDs:	
			#列カラム名出力
			fout_tansho_kaishuritu.write( "\t" + U.getCodeValue( "2202", SexCD, 1 ) )
			fout_fukusho_kaishuritu.write( "\t" + U.getCodeValue( "2202", SexCD, 1 ) )
		...
		for OddsInt in OddsInts:
			...
			for SexCD in SexCDs:
				...
				#単勝回収率
				tansho_kaishuritsu = round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / b_shusokaisu, 1)
				fout_tansho_kaishuritu.write( "\t" + str(tansho_kaishuritsu) )
				#複勝の回収率
				fukusho_kaishuritsu = round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / b_shusokaisu, 1)
		...
		for SexCD in SexCDs:
			...
			for OddsInt in OddsInts:
				...
				total_tansho_haraimodoshi_odds += tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt]
				total_fukusho_haraimodoshi_odds += fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt]
		...
		for SexCD in SexCDs:
			fout_tansho_kaishuritu.write( "\t" + str(race_num[JyoCD][TrackCD+Kyori][SexCD]) )
			fout_fukusho_kaishuritu.write( "\t" + str(race_num[JyoCD][TrackCD+Kyori][SexCD]) )
...
for JyoCD in JyoCDs:
	...
	for Course in Courses:
		...
		for SexCD in SexCDs:
			for OddsInt in OddsInts:
				shusotosu = tyakuKaisu[JyoCD][TrackCD+Kyori][SexCD][OddsInt][0]
				if(shusotosu < MinimumNumber):  continue
				N += 1
				tansho_average += tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / shusotosu
				fukusho_average += fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / shusotosu

				ranking[str(n)] = ( tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / shusotosu + fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / shusotosu ) / 2
				n += 1
...
for t in total_ranking:
	...
	text += "\t" + shibaDirt
	text += "\t" + Kyori + "m"
	text += "\t" + U.getCodeValue( "2202", SexCD, 1 )
	text += "\t" + OddsInt + "~" + str( int(OddsInt) + 1 )
	text += "\t" + str(round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / shusotosu, 1))
	text += "\t" + str(round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][SexCD][OddsInt] / shusotosu, 1))
