...
for JyoCD in JyoCDs:
	...
	for Course in Courses:
		...
		for BaTaijyu in BaTaijyus:
			race_num[JyoCD][TrackCD+Kyori][BaTaijyu] = 0
			tyakuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu] = {}
			tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu] = {}
			fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu] = {}
			tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu] = {}
			fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu] = {}

			for OddsInt in OddsInts:
				#レース結果格納する配列：総数, １着, ２着, ３着
				tyakuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] = [0] * 4
				#単勝・複勝の的中回数
				tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] = 0
				fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] = 0
				#単勝・複勝の払戻総額
				tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] = 0
				fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] = 0
		...
		for Year in Years:
			...
			for RACE in RACEs:
				...
				for UMA_RACE in UMA_RACEs:
					...
					#出走頭数
					race_num[JyoCD][TrackCD+Kyori][BaTaijyu] += 1

					#レース結果を集計
					tyakuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt][0] += 1
					if( int(UMA_RACE["KakuteiJyuni"]) <= 3):
						tyakuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt][int(UMA_RACE["KakuteiJyuni"])] += 1
					#単勝的中の場合
					for i in range(3):
						n = str(i + 1)
						if(HARAI["PayTansyoUmaban" + n] == UMA_RACE["Umaban"]):
							tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] += int(HARAI["PayTansyoPay" + n])
							tansho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] += 1
					#複勝的中の場合
					for i in range(5):
						n = str(i + 1)
						if(HARAI["PayFukusyoUmaban" + n] == UMA_RACE["Umaban"]):
							fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] += int(HARAI["PayFukusyoPay" + n])
							fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] += 1
					#ターミナルへ出力
					#print(text)
			...
			for BaTaijyu in BaTaijyus:
				for OddsInt in OddsInts:
					_tyakuKaisu = tyakuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt]
					#着外の回数
					...
					tansho_kaishuritsu = round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / b_tyakuKaisu, 1)
					#複勝の的中率と回収率
					fukusho_ritsu = round(fukusho_tekityuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / b_tyakuKaisu * 100, 1)
					fukusho_kaishuritsu = round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / b_tyakuKaisu, 1)
	...
	for Course in Courses:
		...
		for BaTaijyu in BaTaijyus:	
			#列カラム名出力
			fout_tansho_kaishuritu.write( "\t" + BaTaijyu + "kg" )
			fout_fukusho_kaishuritu.write( "\t" + BaTaijyu + "kg" )
		...
		for OddsInt in OddsInts:
			...
			for BaTaijyu in BaTaijyus:
				...
				#単勝回収率
				tansho_kaishuritsu = round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / b_shusokaisu, 1)
				fout_tansho_kaishuritu.write( "\t" + str(tansho_kaishuritsu) )
				#複勝の回収率
				fukusho_kaishuritsu = round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / b_shusokaisu, 1)
		...
		for BaTaijyu in BaTaijyus:
			...
			for OddsInt in OddsInts:
				...
				total_tansho_haraimodoshi_odds += tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt]
				total_fukusho_haraimodoshi_odds += fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt]
		...
		for BaTaijyu in BaTaijyus:
			fout_tansho_kaishuritu.write( "\t" + str(race_num[JyoCD][TrackCD+Kyori][BaTaijyu]) )
			fout_fukusho_kaishuritu.write( "\t" + str(race_num[JyoCD][TrackCD+Kyori][BaTaijyu]) )
...
for JyoCD in JyoCDs:
	...
	for Course in Courses:
		...
		for BaTaijyu in BaTaijyus:
			for OddsInt in OddsInts:
				shusotosu = tyakuKaisu[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt][0]
				if(shusotosu < MinimumNumber):  continue
				N += 1
				tansho_average += tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / shusotosu
				fukusho_average += fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / shusotosu

				ranking[str(n)] = ( tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / shusotosu + fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / shusotosu ) / 2
				n += 1
...
for t in total_ranking:
	...
	text += "\t" + str(round(tansho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / shusotosu, 1))
	text += "\t" + str(round(fukusho_haraimodoshi_sum[JyoCD][TrackCD+Kyori][BaTaijyu][OddsInt] / shusotosu, 1))
